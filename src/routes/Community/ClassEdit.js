...
export default class ClassAdd extends PureComponent {
  ...
  handleSubmit = () => {
    const { classModel: { classForm: { title, xxx, list, yyy, desc, detail }, editClass } } = this.props;
  ...
  // };
  render() {
    const {  classModel: { classForm }, uploadUrl, live: { vodListPage, vodList } } = this.props;
    const { header, isVodModal } = this.state;
    const vodListItem = [];
    vodList.forEach(res => {
      vodListItem.push(
